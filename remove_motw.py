...
def remove_mark_of_the_web_from_shortcuts(desktop_path: str):
    ...
    if not os.path.isdir(desktop_path):
        print(f"Desktop folder not found for user profile: {desktop_path}")
        return

    print(f"Starting Mark of the Web removal in: {desktop_path}")
    ...
    def _remove_ads(file_path: str):
        ...
        try:
            ...
            print(f"  Successfully removed MotW from: {file_path} (via os.remove)")
        except FileNotFoundError:
            print(f"  No Mark of the Web found for: {file_path}")
        except OSError as e:
            print(f"  os.remove failed for {file_path}: {e}. Trying 'del' command.")
            try:
                ...
                print(f"  Subprocess error for {file_path}: {sub_e}")
        ...
        for file_name in files:
            ...
            if ext in (".url", ".lnk"):
                if has_zone_identifier_ads(file_path):
                    print(f"Processing shortcut with MotW: {file_name}")
                    _remove_ads(file_path)
                else:
                    print(f"Skipping (no MotW): {file_name}")

    print(f"Finished Mark of the Web removal for: {desktop_path}")
